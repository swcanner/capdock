...
class Tor_Model(pl.LightningModule):
    def __init__(
        ...
        if self.perturb_tor:
            self.tor_diffuser = SO3_TorDiffuser(diffuser.so3)
    ...
    def loss_fn(self, rec_x, lig_x, rec_pos, lig_pos, contact_matrix, position_matrix, eps=1e-5):
        with torch.no_grad():
            ...
            if self.perturb_tor:
                tor_score_scale = self.so3_diffuser.score_scaling(t.item())
                tor_update, rot_score_gt = self.so3_diffuser.forward_marginal(t.item())
                tor_update = torch.from_numpy(rot_update).float().to(self.device)
                tor_score_gt = torch.from_numpy(rot_score_gt).float().to(self.device)
            else:
